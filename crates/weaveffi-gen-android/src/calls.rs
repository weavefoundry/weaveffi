//! Per-callable Kotlin wrappers: free functions, `suspend fun` shims,
//! listener registration, and the interface-member native declarations.
...
use crate::types::{
    camel_params, kotlin_cb_jni_type, kotlin_cb_type, kotlin_fn_name, kotlin_iterator_class_name,
    kotlin_jni_type, kotlin_type, kt_param, needs_wrapper_split,
};
...
/// Render one listener's registration surface into the `WeaveFFI` companion:
/// the register external (behind a decoding wrapper when the callback carries
/// buffered arguments) and the unregister external, with the exception policy
