...
mod tests {
    ...
    fn capabilities_includes_callbacks_and_listeners() {
        ...
        assert!(
            ...
            "Android generator must advertise Callbacks now that callback codegen is implemented"
        );
        assert!(
            caps.contains(&Capability::Listeners),
            "Android generator must advertise Listeners now that listener codegen is implemented"
        );
