...
impl DotnetGenerator {
    fn generate_impl(&self, api: &Api, out_dir: &Utf8Path, namespace: &str) -> Result<()> {
        let dir = out_dir.join("dotnet");
        std::fs::create_dir_all(&dir)?;
        std::fs::write(
            dir.join(format!("{namespace}.cs")),
            render_csharp(api, namespace),
        )?;
        std::fs::write(
...
impl Generator for DotnetGenerator {
    ...
    fn generate_with_config(
        ...
        self.generate_impl(api, out_dir, config.dotnet_namespace())
    }
...
fn render_csharp(api: &Api, namespace: &str) -> String {
...
mod tests {
    ...
    fn simple_i32_function() {
        ...
        let cs = render_csharp(&api, "WeaveFFI");
