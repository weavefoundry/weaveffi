...
fn render_native_methods(out: &mut String, api: &Api) {
    ...
    for m in &api.modules {
        ...
        for f in &m.functions {
            ...
            if f.r#async {
                render_async_function_pinvoke(out, &m.name, f);
            }
        }
    }
...
fn render_wrapper_method(out: &mut String, m: &Module, f: &Function, strip_module_prefix: bool) {
    if f.r#async {
        render_async_wrapper_method(out, m, f, strip_module_prefix);
        return;
    }
...
fn render_async_wrapper_method(
    ...
    if needs_try {
        ...
        out.push_str("            try\n            {\n");
        out.push_str(&format!(
            "                NativeMethods.{c_sym}_async({args_part}callback, IntPtr.Zero);\n"
        ));
        out.push_str("            }\n            finally\n            {\n");
        for p in &f.params {
            render_marshal_cleanup(out, p, "                ");
